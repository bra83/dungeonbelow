...
function App() {
    ...
    const handleInstallClick = () => {
        if (deferredPrompt) {
            ...
            // Se o prompt não estiver disponível (ex: iOS ou já instalado/rejeitado), mostra instrução
            ...
            <div className="view-container">
                ...
                <div className="grid-cards" style={{gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 24}}>
                    ...
                    <div style={{display:'flex', flexDirection:'column', gap: 24}}>
                        {/* FUNIL DE VENDAS */}
                        <div className="neu-card">
                            <h3 style={{fontSize: '0.9rem', color: 'var(--text-light)', textTransform: 'uppercase', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8}}>
                    ...
                    {sortedQuotes.map(q => (
                        <div key={q.id} className="neu-card" style={{position:'relative'}}>
                            ...
                            <div className="card-actions" style={{display:'flex', gap: 8}}>
                                ...
                                {q.status !== 'approved' && (
                                    <button className="neu-btn primary" title="Aprovar Venda" onClick={() => handleApproveQuote(q.id)}>
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
                                    </button>
                                )}
                                <button className="neu-btn danger" title="Excluir" onClick={() => handleDeleteQuote(q.id)}>
                                ...
                                <tr key={e.id}>
                                    ...
                                    <td style={{textAlign: 'center'}}>
                                         <div style={{display:'flex', gap: 5, justifyContent:'center'}}>
                                            <button className="mini-btn" onClick={() => { setEditingId(e.id); setDrawerType('expense'); setDrawerOpen(true); }}>
                                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                                            </button>
                                            <button className="mini-btn danger" onClick={() => handleDeleteExpense(e.id)}>
